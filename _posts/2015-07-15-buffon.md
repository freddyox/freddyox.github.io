...
effectively setting up a Monte Carlo simulation, and recording the number of times
a needle intersects (denoted by <i>N<sub>int</sub></i>) with a line on the poster board.
The probability that the needle intersects with a line on the
poster board, or <i>N<sub>int</sub></i>/<i>N</i>, may be used to estimate &pi;.
...
parameters are unintentially tweaked (or randomly selected), and all the experimenter cares about is the result.
